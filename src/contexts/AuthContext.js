...
const BACKEND_URL =
  window.location.hostname === "localhost"
    ? "http://localhost:8000"
    : "https://manovaani.manomantapa.com/backend-otp";
...
export const AuthProvider = ({ children }) => {
  ...
  const [subscriptionPlan, setSubscriptionPlan] = useState(() => {
    if (currentUser && currentUser.subscription_plan)
      return currentUser.subscription_plan;
  ...
  const signIn = async (userObj) => {
    try {
      ...
      const response = await axios.get(
        `${BACKEND_URL}/get-user?email=${encodeURIComponent(userObj.email)}`
      );
  ...
  const updateUser = async (userData) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        return {
          success: false,
          error: response.data.message || "Update failed on server.",
        };
      }
    } catch (error) {
      const serverMessage = error.response?.data?.message || error.message;
